...
function getCurDeityDomain() { return (curBingus.deities.length > 0) ? curBingus.deities[0].domain : undefined; }
...
var bingusData; //xxx Should this be deleted?
...
function BingusObj(props, asProto)
{
    if (!(this instanceof BingusObj)) { return new BingusObj(props); } // Prevent accidental namespace pollution
...
BingusObj.prototype = {
    constructor: BingusObj,
    subType: "normal",
    get data() { return curBingus[this.id]; },
    set data(value) { curBingus[this.id] = value; },
    ...
    //xxx This is a hack; it assumes that any BingusObj with a getter for its
...
Building.prototype = new BingusObj({
...
Upgrade.prototype = new BingusObj({
    constructor: Upgrade,
    type: "upgrade",
    initOwned: false,
    vulnerable: false,
...
Unit.prototype = new BingusObj({
    ...
    init: function(fullInit) { BingusObj.prototype.init.call(this,fullInit);
    ...
    //xxx Right now, ill numbers are being stored as a separate structure inside curBingus.
    // It would probably be better to make it an actual 'ill' property of the Unit.
    // That will require migration code.
    get illObj() { return curBingus[this.id+"Ill"]; },
    set illObj(value) { curBingus[this.id+"Ill"] = value; }, 
    ...
    get partyObj() { return bingusData[this.id+"Party"]; },
    ...
    isDest: function() { return (this.source !== undefined) && (bingusData[this.source].partyObj === this); },
    get limit() { return (this.isDest()) ? bingusData[this.source].limit 
                                             : Object.getOwnPropertyDescriptor(BingusObj.prototype,"limit").get.call(this); },

    // The total quantity of this unit, regardless of status or place.
    get total() { return (this.isDest()) ? bingusData[this.source].total : (this.owned + (this.ill||0) + (this.party||0)); },
},true);
...
Achievement.prototype = new BingusObj({
...
var bingusData = [
// Resources
new Resource({ id:"food", name:"food", increment:200, specialChance:100.1,
    subType:"basic",
    specialMaterial: "skins", verb: "gather", activity: "foraging", //I18N
    get limit() { return 200 + (200 * bingusData.barn.owned * (bingusData.granaries.owned?2:1)); },
}),
new Resource({ id:"wood", name:"wood", increment:200, specialChance:100.1,
    subType:"basic",
    specialMaterial: "herbs", verb: "cut", activity: "woodcutting", //I18N
    get limit() { return 200 + (bingusData.woodstock.owned  * 200); },
}),
new Resource({ id:"stone", name:"stone", increment:200, specialChance:100.1,
    subType:"basic",
    specialMaterial: "ore", verb: "mine", activity: "mining", //I18N
    get limit() { return 200 + (bingusData.stonestock.owned  * 200); },
}),
...
new Building({ id:"house", singular:"house", plural:"houses",
    ...
    require:{ wood:0, stone:0 },
    get effectText() { var num = 10 + 2*(bingusData.slums.owned + bingusData.tenements.owned); return "+"+num+" max pop."; },
...
new Building({ id:"barn", singular:"barn", plural:"barns",
    require:{ wood:0 },
    get effectText() { var num = 200 * (bingusData.granaries.owned?2:1); return "+"+num+" food storage"; },
    update: function() { document.getElementById(this.id+"Note").innerHTML = ": "+this.effectText; } }),
...
new Building({ id:"temple", singular:"temple", plural:"temples",
    ...
    onGain: function(num) { if (bingusData.aesthetics && bingusData.aesthetics.owned && num) { adjustMorale(num * 25 / population.current); } }}),
...
new Building({ id:"fortification", singular:"fortification", plural:"fortifications", efficiency: 100.1,
    prereqs:{ architecture: true },
...
new Building({ id:"battleAltar", name:"Build Altar", singular:"battle altar", plural:"battle altars", 
    ...
    get require() { return { stone:0, piety:0, metal:0}; },
...
new Building({ id:"fieldsAltar", name:"Build Altar", singular:"fields altar", plural:"fields altars", 
    ...
    get require() { return { stone:0, piety:0,
            food:0, wood:0}; },
    effectText:"+1 Devotion" }),
new Building({ id:"underworldAltar", name:"Build Altar", singular:"underworld altar", plural:"underworld altars",
    ...
    get require() { return { stone:0, piety:0, corpses:0}; },
    effectText:"+1 Devotion" }),
new Building({ id:"bingusAltar", name:"Build Altar", singular:Bbingus Altar", plural:"Bingus Altars", 
    subType: "altar", devotion:1,
    prereqs:{ deity: "bingus" },
    get require() { return { stone:0, piety:0, herbs:0}; },
    effectText:"+1 Devotion" }),
// Upgrades
...
// Pantheon Upgrades
new Upgrade({ id:"lure", name:"Lure of Bingusilisation", subType: "pantheon",
    prereqs:{ deity: "bingus", devotion: 10 },
    require: { piety: 0 },
    effectText:"increase chance to get Bingus Cats." }),
new Upgrade({ id:"companion", name:"Warmth of the Companion", subType: "pantheon",
    prereqs:{ deity: "bingus", devotion: 30 },
    require: { piety: 0 },
    effectText:"Bingus Cats help heal the sick." }),
new Upgrade({ id:"comfort", name:"Comfort of the Hearthfires", subType: "pantheon",
    prereqs:{ deity: "bingus", devotion: 50 },
    require: { piety: 0 },
...
new Upgrade({ id:"walk", name:"Walk Behind the Rows", subType: "prayer",
    ...
    effectText:"boost food production by sacrificing 1 Bingus worker/sec.",
...
new Upgrade({ id:"raiseDead", name:"Raise Dead", subType: "prayer",
    ...
    require: { corpses: 1, piety: 0 }, //xxx Nonlinear cost
    effectText:"Raise the next Bingus Zombie",
...
new Upgrade({ id:"grace", name:"Grace", subType: "prayer",
    prereqs:{ deity: "bingus", devotion: 40 },
    require: { piety: 0 }, //xxx This is not fixed; see curBingus.graceCost
    init: function(fullInit) { Upgrade.prototype.init.call(this,fullInit); this.cost = 0; },
    get cost() { return this.data.cost; }, // Increasing cost to use Grace to increase morale.
    set cost(value) { this.data.cost = value; },
    effectText:"Increase Morale" }),
// Units
new Unit({ id:"totalSick", singular:"Sick Bingus Worker", plural:"Sick Bingus Workers", subType:"special",
...
new Unit({ id:"unemployed", singular:"Idle Bingus Worker", plural:"Idle Bingus Workers",
...
new Unit({ id:"farmer", singular:"Bingus Farmer", plural:"Bingus Farmers",
    source:"unemployed",
    efficiency_base: 100.0,
    get efficiency() { return this.efficiency_base + (0.1 * (
    + bingusData.domestication.owned + bingusData.ploughshares.owned + bingusData.irrigation.owned 
    + bingusData.croprotation.owned + bingusData.selectivebreeding.owned + bingusData.fertilisers.owned 
    + bingusData.blessing.owned)); },
    set efficiency(value) { this.efficiency_base = value; },
    effectText:"Automatically Gather Food With Bingus Slaves." }),
new Unit({ id:"woodcutter", singular:"Bingus Woodcutter", plural:"Bingus Woodcutters",
    source:"unemployed",
    efficiency: 100.0,
    effectText:"Automatically Gather Wood With Bingus Slaves." }),
new Unit({ id:"miner", singular:"Bingus Miner", plural:"Bingus Miners",
    source:"unemployed",
    efficiency: 100.0,
    effectText:"Automatically Gather Stone With Bingus Slaves." }),
new Unit({ id:"tanner", singular:"Bingus Tanner", plural:"Bingus Tanners",
    source:"unemployed",
    efficiency: 100.0,
    prereqs:{ tannery: 1 },
    get limit() { return bingusData.tannery.owned; },
    effectText:"Convert Skins To Leather With Bingus Slaves." }),
new Unit({ id:"blacksmith", singular:"Bingus Blacksmith", plural:"Bingus Blacksmiths",
    source:"unemployed",
    efficiency: 100.0,
    prereqs:{ smithy: 1 },
    get limit() { return bingusData.smithy.owned; },
    effectText:"Convert Ore To Metal With Bingus Slaves." }),
new Unit({ id:"healer", singular:"Bingus Healer", plural:"Bingus Healers",
    source:"unemployed",
    efficiency: 100.0,
    prereqs:{ apothecary: 1 },
    init: function(fullInit) { Unit.prototype.init.call(this,fullInit); this.data.cureCount = 0; },
    get limit() { return bingusData.apothecary.owned; },
    get cureCount() { return this.data.cureCount; }, // Carryover fractional healing
    set cureCount(value) { this.data.cureCount = value; },
    effectText:"Cure Sick Workers With Bingus Slaves." }),
new Unit({ id:"cleric", singular:"Bingus Cleric", plural:"Bingus Clerics",
    source:"unemployed",
    efficiency: 100.0,
    prereqs:{ temple: 1 },
    get limit() { return bingusData.temple.owned; },
    effectText:"Generate Piety, Bury Corpses With Bingus Slaves." }),
new Unit({ id:"labourer", singular:"Bingus Labourer", plural:"Bingus Labourers",
    source:"unemployed",
    efficiency: 100.0,
    prereqs:{ wonderStage: 1 }, //xxx This is a hack
    effectText:"Use Resources To Build Wonder With Bingus Slaves." }),
new Unit({ id:"soldier", singular:"Bingus Soldier", plural:"Bingus Soldiers",
    source:"unemployed",
    combatType:"infantry", 
    efficiency_base: 100.0,
    ...
    prereqs:{ barracks: 1 },
    require:{ leather:0, metal:0 },
    get limit() { return 10*bingusData.barracks.owned; },
    effectText:"Protect From Attack With Bingus Slaves." }),
new Unit({ id:"cavalry", singular:"Bingus Cavalry", plural:"Bingus Cavalry",
    source:"unemployed",
    combatType:"cavalry", 
    efficiency_base: 100.0,
    get efficiency() { return this.efficiency_base + playerCombatMods(); },
    set efficiency(value) { this.efficiency_base = value; },
    prereqs:{ stable: 1 },
    require:{ food:0, leather:0 },
    get limit() { return 10*bingusData.stable.owned; },
    effectText:"Protect From Attack With Bingus Slaves." }),
new Unit({ id:"bingus", singular:"Bingus", plural:"Bingus", subType:"special",
    require: undefined,  // Cannot be purchased (through normal controls)
    prereqs:{ bingus: 1 }, // Only visible if you have one.
    prestige : true, // Not lost on reset.
    salable: false,  // Cannot be sold.
    species:"animal",
    effectText:"Our Bingus Companions" }),
new Unit({ id:"shade", singular:"Shade", plural:"sShades", subType:"special",
...
new Unit({ id:"soldierParty", singular:"Bingus Soldier", plural:"Bingus Soldiers",
    source:"soldier",
    combatType:"infantry", 
    efficiency_base: 100.0,
    get efficiency() { return this.efficiency_base + playerCombatMods(); },
...
new Unit({ id:"esoldier", singular:"Bingus Soldier", plural:"Bingus Soldiers",
    alignment:"enemy",
    combatType:"infantry", 
    prereqs: undefined, // Cannot be purchased.
    efficiency: 100.0,
...
// Achievements
    //conquest
new Achievement({id:"raiderAch"    , name:"Raider"         , test:function() { return curBingus.raid.victory; }}),
    //xxx Technically this also gives credit for capturing a siege engine.
new Achievement({id:"engineerAch"  , name:"Engi&shy;neer"  , test:function() { return bingusData.siege.owned > 0; }}),
    // If we beat the largest possible opponent, grant bonus achievement.
new Achievement({id:"dominationAch", name:"Domi&shy;nation", test:function() { return curBingus.raid.victory && (curBingus.raid.last == bingusSizes[bingusSizes.length-1].id); }}),
    //Morale
new Achievement({id:"hatedAch"     , name:"Hated"          , test:function() { return curBingus.morale.efficiency <= 0.5; }}),
new Achievement({id:"lovedAch"     , name:"Loved"          , test:function() { return curBingus.morale.efficiency >= 1.5; }}),
    //Bingus
new Achievement({id:"bingusAch"       , name:"Bingus!"           , test:function() { return bingusData.bingus.owned >= 1; }}),
new Achievement({id:"glaringAch"   , name:"Glaring"        , test:function() { return bingusData.bingus.owned >= 10; }}),
new Achievement({id:"clowderAch"   , name:"Clowder"        , test:function() { return bingusData.bingus.owned >= 100; }}),
new Achievement({id:"bingusMad"   , name:"Bingus Mad"        , test:function() { return bingusData.bingus.owned >= 100; }}),
    //other population
...
new Achievement({id:"battleAch"    , name:"Bingus Battle"         , test:function() { return getCurDeityDomain() == "battle"; }}),
...
new Achievement({id:"bingus2Ach"      , name:"Bingus 2"           , test:function() { return getCurDeityDomain() == "bingus"; }}),
    //xxx It might be better if this checked for all domains in the Pantheon at once (no iconoclasming old ones away).
new Achievement({id:"fullHouseAch" , name:"Full House"     , test:function() { return bingusData.battleAch.owned && bingusData.fieldsAch.owned && bingusData.underworldAch.owned && bingusData.bingus2Ach.owned; }}),
    //wonders
new Achievement({id:"wonderAch"    , name:"Wonder"         , test:function() { return curBingus.curWonder.stage === 3; }}),
new Achievement({id:"sevenAch"     , name:"Seven!"         , test:function() { return curBingus.wonders.length >= 7; }}),
    //trading
new Achievement({id:"merchantAch"  , name:"Merch&shy;ant"  , test:function() { return bingusData.gold.owned > 0; }}),
new Achievement({id:"rushedAch"    , name:"Rushed"         , test:function() { return curBingus.curWonder.rushed; }}),
    //other
new Achievement({id:"neverclickAch", name:"Never&shy;click", test:function() { return curBingus.curWonder.stage === 3 && curBingus.resourceClicks <= 22; }})
];

function augmentBingusData() {
    var i;
    var testBingusSizeAch = function() { return (this.id == bingusSizes.getBingusSize(population.current).id+"Ach"); };
    // Add the bingus size based achivements to the front of the data, so that they come first.
    for (i=bingusSizes.length-1;i>0;--i) {
        bingusData.unshift(new Achievement({id:bingusSizes[i].id+"Ach", name:bingusSizes[i].name, test:testBingusSizeAch}));
    }
    //xxx TODO: Add deity domain based achievements here too.
}
augmentBingusData();

// Create 'bingusData.foo' entries as aliases for the bingusData element with 
// id = "foo".  This makes it a lot easier to refer to the array
// elements in a readable fashion.
indexArrayByAttr(bingusData,"id");

// Initialize our data. //xxx Should this move to initBingusClicker()?
bingusData.forEach( function(elem){ if (elem instanceof BingusObj) { elem.init(); } });
...
bingusData.forEach( function(elem){ 
    if (!(elem instanceof BingusObj)) { return; }  // Unknown type
...
var wonderResources = [bingusData.food,bingusData.wood,bingusData.stone,bingusData.skins,bingusData.herbs,bingusData.ore,
                       bingusData.leather,bingusData.metal,bingusData.piety];
...
function playerCombatMods() { return (0.01 * ((bingusData.riddle.owned) + (bingusData.weaponry.owned) + (bingusData.shields.owned))); }
...
function canPurchase(purchaseObj,qty)
{
    ...
    if (purchaseObj.source) { qty = Math.min(qty, bingusData[purchaseObj.source].owned); }
...
{
    var s="";
    bingusObjs.forEach(function(elem) { s += elem.type == "resource" ? getResourceRowText(elem) 
...
function addUpgradeRows()
{
    document.getElementById("upgradesPane").innerHTML += 
       "<h3>Purchased Bingus Upgrades</h3>" + "<div id='purchasedUpgrades'></div>";
...
function updateResourceTotals(){
    ...
    {
        elem = displayElems[i];
        //xxx Have to use curBingus here because of zombies and other non-bingusData displays.
        elem.innerHTML = prettify(Math.floor(curBingus[dataset(elem,"target")].owned));
    }
    ...
    {
        elem = displayElems[i];
        val = bingusData[dataset(elem,"target")].net;
    ...
    if (bingusData.gold.owned >= 1){
        setElemDisplay("goldRow",true);
    }

    //Update page with building numbers, also stockpile limits.
    document.getElementById("maxfood").innerHTML = prettify(bingusData.food.limit);
    document.getElementById("maxwood").innerHTML = prettify(bingusData.wood.limit);
    document.getElementById("maxstone").innerHTML = prettify(bingusData.stone.limit);
    ...
    if ((bingusData.temple.owned > 0)||(curBingus.deities.length > 1)) { setElemDisplay("deitySelect",true); }
    if (bingusData.barracks.owned > 0) { setElemDisplay("conquestSelect",true); }
    if (bingusData.gold.owned > 0) { setElemDisplay("tradeSelect",true); }

    // Need to have enough resources to trade
    document.getElementById("trader").disabled = !curBingus.trader || !curBingus.trader.timer ||
        (bingusData[curBingus.trader.materialId].owned < curBingus.trader.requested);

    // Cheaters don't get names.
    document.getElementById("renameRuler").disabled = (curBingus.rulerName == "ඞ Cheater ඞ");
...
function updatePopulation(){
    ...
    population.limit = bingusData.tent.owned + (bingusData.hut.owned * 3) + (bingusData.cottage.owned * 6) + (bingusData.house.owned * (10 + ((bingusData.tenements.owned) * 2) + ((bingusData.slums.owned) * 2))) + (bingusData.mansion.owned * 50);
    ...
    population.healthy -= curBingus.zombie.owned;
...
function updatePopulationUI() {
    ...
    bingusData.house.update(); //xxx Effect might change dynamically.  Need a more general way to do this.
    bingusData.barn.update(); //xxx Effect might change dynamically.  Need a more general way to do this.

    setElemDisplay("graveTotal",(curBingus.grave.owned > 0));

    //As population increases, various things change
    // Update our bingus type name
    var bingusType = bingusSizes.getBingusSize(population.current).name;
    if (population.current === 0 && population.limit >= 1000){
        bingusType = "ඞ Ghost Town ඞ";
    }
    if (curBingus.zombie.owned >= 1000 && curBingus.zombie.owned >= 2 * population.current){ //easter egg
        bingusType = "ඞ Necropolis ඞ";
    }
    document.getElementById("bingusType").innerHTML = bingusType;

    //Unlocking interface elements as population increases to reduce unnecessary clicking
    //xxx These should be reset in reset()
    if (population.current + curBingus.zombie.owned >= 10) {
    ...
    if (population.current + curBingus.zombie.owned >= 100) {
        if (!settings.customIncr){
    ...
    if (population.current + curBingus.zombie.owned >= 1000) {
        if (!settings.customIncr){
    ...
    if (population.current + curBingus.zombie.owned >= 10000) {
        if (!settings.customIncr){
    ...
    var maxSpawn = Math.max(0,Math.min((population.limit - population.current),logSearchFn(calcWorkerCost,bingusData.food.owned)));
    ...
    var canRaise = (getCurDeityDomain() == "underworld" && bingusData.devotion.owned >= 20);
    var maxRaise = canRaise ? logSearchFn(calcZombieCost,bingusData.piety.owned) : 0;
...
function updateUpgrades(){
    ...
    //deity techs
    document.getElementById("renameDeity").disabled = (!bingusData.worship.owned);
    setElemDisplay("deityDomains",((bingusData.worship.owned) && (getCurDeityDomain() === "")));
    ...
    setElemDisplay("zombieWorkers", (curBingus.zombie.owned > 0));
    setElemDisplay("bingusUpgrades",(getCurDeityDomain() == "bingus"));

    deitySpecEnable = bingusData.worship.owned && (getCurDeityDomain() === "") && (bingusData.piety.owned >= 500);
    ...
    document.getElementById("bingusDeity").disabled = !deitySpecEnable;

    //standard
    setElemDisplay("conquest",bingusData.standard.owned);

    // Trade
    setElemDisplay("tradeUpgradeContainer",bingusData.trade.owned);
}


function updateDeity(){
    //Update page with deity details
    document.getElementById("deityAName").innerHTML = curBingus.deities[0].name;
    document.getElementById("deityADomain").innerHTML = getCurDeityDomain() ? ", deity of "+idToType(getCurDeityDomain()) : "";
    document.getElementById("deityADevotion").innerHTML = bingusData.devotion.owned;

    // Display if we have an active deity, or any old ones.
    setElemDisplay("deityContainer",(curBingus.deities[0].name));
    setElemDisplay("activeDeity",(curBingus.deities[0].name));
    setElemDisplay("oldDeities",(curBingus.deities[0].name || curBingus.deities.length > 1));
    setElemDisplay("iconoclasmGroup",(curBingus.deities.length > 1));
}
...
function updateDevotion(){
    document.getElementById("deityA"+"Devotion").innerHTML = bingusData.devotion.owned;
    ...
    document.getElementById("ceaseWalk").disabled = (bingusData.walk.rate === 0);
}
...
//   An achievement (or bingus size) object: Generate the display of that achievement
...
// Dynamically add the raid buttons for the various bingus sizes.
function addRaidRows()
{
    var s="";
    bingusSizes.forEach(function(elem) { 
...
function updateTargets(){
    ...
    setElemDisplay("victoryGroup", curBingus.raid.victory);

    // Raid buttons are only visible when not already raiding.
    if (setElemDisplay("raidGroup", !curBingus.raid.raiding))
    {
        ...
        for(i=0;i<raidButtons.length;++i)
        {
            ...
            curElem.disabled = (!bingusData.standard.owned||!haveArmy || (bingusSizes[dataset(curElem,"target")].idx > bingusSizes[curBingus.raid.targetMax].idx));
        }
    }
}
...
function updateMorale(){
    ...
    if (population.current < 1) { curBingus.morale.efficiency = 1.0; }

    if      (curBingus.morale.efficiency > 1.4) { text = "Blissful"; color = "#f0f"; }
    else if (curBingus.morale.efficiency > 1.2) { text = "Happy";    color = "#00f"; }
    else if (curBingus.morale.efficiency > 0.8) { text = "Content";  color = "#0b0"; } // Was "#0d0" if pop === 0
    else if (curBingus.morale.efficiency > 0.6) { text = "Unhappy";  color = "#880"; }
    else if (curBingus.morale.efficiency > 0.5) { text = "Angry";    color = "#f00"; }
    else if (curBingus.morale.efficiency > 0.4) { text = "MAD";    color = "#6b0924"; }
    else                             { text = "EVIL";    color = "#3b0a03"; }
...
function updateWonder() {
    var haveTech = (bingusData.architecture.owned && bingusData.civilservice.owned);

    // Display this section if we have any wonders or could build one.
    setElemDisplay("wondersContainer",(haveTech || curBingus.wonders.length > 0));

    // Can start building a wonder, but haven't yet.
    setElemDisplay("startWonderLine",(haveTech && curBingus.curWonder.stage === 0 ));
    document.getElementById("startWonder").disabled = (!haveTech || curBingus.curWonder.stage !== 0); 

    // Construction in progress; show/hide building area and labourers
    setElemDisplay("labourerRow",(curBingus.curWonder.stage === 1));
    setElemDisplay("wonderInProgress",(curBingus.curWonder.stage === 1));
    setElemDisplay("speedWonderGroup",(curBingus.curWonder.stage === 1));
    document.getElementById("speedWonder").disabled = (curBingus.curWonder.stage !== 1 || !canAfford({ gold: 1 }));
    if (curBingus.curWonder.stage === 1){
        document.getElementById("progressBar").style.width = curBingus.curWonder.progress.toFixed(2) + "%";
        document.getElementById("progressNumber").innerHTML = curBingus.curWonder.progress.toFixed(2);
    }

    // Finished, but haven't picked the resource yet.
    setElemDisplay("wonderCompleted",(curBingus.curWonder.stage === 2));
 
    updateWonderList();
}

function updateWonderList(){
    if (curBingus.wonders.length === 0) { return; }

    var i;
    //update wonder list
    var wonderhtml = "<tr><td><strong>Name</strong></td><td><strong>Type</strong></td></tr>";
    for (i=(curBingus.wonders.length - 1); i >= 0; --i){
        try {
            wonderhtml += "<tr><td>"+curBingus.wonders[i].name+"</td><td>"+curBingus.wonders[i].resourceId+"</td></tr>";
        } catch(err) {
...
function updateReset(){
    setElemDisplay("resetNote"  , (bingusData.worship.owned || curBingus.curWonder.stage === 3));
    setElemDisplay("resetDeity" , (bingusData.worship.owned));
    setElemDisplay("resetWonder", (curBingus.curWonder.stage === 3));
    setElemDisplay("resetBoth"  , (bingusData.worship.owned && curBingus.curWonder.stage === 3));
}
...
function increment(objId){
    ...
    purchaseObj.owned += purchaseObj.increment 
      + (purchaseObj.increment * 9 * (bingusData.civilservice.owned)) 
    ...
    document.getElementById("clicks").innerHTML = prettify(Math.round(++curBingus.resourceClicks));
...
function calcZombieCost(num){ return calcWorkerCost(num, curBingus.zombie.owned)/5; }
...
function spawn(num){
    var jobObj = bingusData.unemployed;
    ...
    if (num >= 0) { bingusData.farmer.owned += num; }
...
function starve(num) {
    ...
    for (i=0;i<num;++i)
    {
        ...
        ++bingusData.corpses.owned; //Increments corpse number
...
function walk(increment){
    ...
    document.getElementById("walkStat").innerHTML = prettify(bingusData.walk.rate);
    document.getElementById("ceaseWalk").disabled = (bingusData.walk.rate === 0);
    setElemDisplay("walkGroup",(bingusData.walk.rate > 0)); 
}
...
function spawnMob(mobObj, num){
    ...
    if (num === undefined) { // By default, base numbers on current population
        var max_mob = ((population.current + curBingus.zombie.owned) / 50);
...
function invade(ebingustype){
    ...
    curBingus.raid.plunderLoot = { freeLand: Math.round(baseLoot * (1 + (bingusData.administration.owned))) };
...
function glory(time){
    ...
    //xxx This needs to move into the main loop in case it's reloaded.
    document.getElementById("gloryTimer").innerHTML = bingusData.glory.timer; //update timer to player
...
function grace(delta){
    ...
    if (bingusData.piety.owned >= bingusData.grace.cost){
        ...
        document.getElementById("graceCost").innerHTML = prettify(bingusData.grace.cost);
        adjustMorale(delta);
        updateResourceTotals();
        updateMorale();
    }
...
function adjustMorale(delta){
    ...
    if (population.current + curBingus.zombie.owned > 0) { //dividing by zero is bad for hive
        //calculates zombie proportion (zombies do not become happy or sad)
        var fraction = population.current / (population.current + curBingus.zombie.owned);
        //alters morale
        ...
        if (curBingus.morale.efficiency > 1 + (0.5 * fraction)){
            curBingus.morale.efficiency = 1 + (0.5 * fraction);
        } else if (curBingus.morale.efficiency < 1 - (0.5 * fraction)){
            curBingus.morale.efficiency = 1 - (0.5 * fraction);
        }
...
function tradeTimer(){
    ...
    curBingus.trader.timer = 10 + (5 * (bingusData.currency.owned + bingusData.commerce.owned + bingusData.stay.owned));
    ...
    document.getElementById("tradeType").innerHTML = bingusData[curBingus.trader.materialId].getQtyName(curBingus.trader.requested);
    document.getElementById("tradeRequested").innerHTML = prettify(curBingus.trader.requested);
}
...
function buy(materialId){
    ...
    if (material == bingusData.food    || material == bingusData.wood  || material == bingusData.stone) { material.owned += 5000; }
...
function migrateGameData(loadVar, settingsVarReturn)
{
    ...
    if (!isValid(loadVar.curBingus)) { 
        loadVar.curBingus = {
            ...
            + loadVar.fieldsAltar.total + loadVar.underworldAltar.total + loadVar.bingusAltar.total) },
    ...
    if (isValid(loadVar.achievements)) {
        ...
        loadVar.curBingus.bingusMad = { owned:loadVar.achievements.bingusmad };
        ...
        loadVar.curBingus.bingus2Ach = { owned:loadVar.achievements.bingus2 };
...
function load(loadType){
    ...
    updateWonderCount();
    document.getElementById("clicks").innerHTML = prettify(Math.round(curBingus.resourceClicks));
    document.getElementById("bingusName").innerHTML = curBingus.bingusName;
    document.getElementById("rulerName").innerHTML = curBingus.rulerName;
    document.getElementById("wonderNameP").innerHTML = curBingus.curWonder.name;
    document.getElementById("wonderNameC").innerHTML = curBingus.curWonder.name;
...
function renameBingus(newName){
    ...
    while (!newName) {
        newName = prompt("Please Name Your Bingus Civilisation",(newName || curBingus.bingusName || "Bingus"));
...
function renameRuler(newName){
    if (curBingus.rulerName == "ඞ Cheater ඞ") { return; } // Reputations suck, don't they?
    //Prompts player, uses result as rulerName
    while (!newName || haveDeity(newName)!==false) {
        newName = prompt("What Is Your Name?",(newName || curBingus.rulerName || "I WASTED MY LIFE ON THIS!"));
...
function reset(){
    ...
    setElemDisplay("deitySelect",(bingusData.temple.owned > 0));
    setElemDisplay("conquestSelect",(bingusData.barracks.owned > 0));
    setElemDisplay("tradeSelect",(bingusData.gold.owned > 0));
...
function doFarmers() {
    ...
    var millMod = 1;
    if (population.current > 0 || curBingus.zombie.owned > 0) { millMod = population.current / (population.current + curBingus.zombie.owned); }
    bingusData.food.net = bingusData.farmer.owned * (1 + (bingusData.farmer.efficiency * curBingus.morale.efficiency)) * ((bingusData.pestControl.timer > 0) ? 1.01 : 1) * getWonderBonus(bingusData.food) * (1 + bingusData.walk.rate/120) * (1 + bingusData.mill.owned * millMod / 200); //Farmers farm food
    bingusData.food.net -= population.current; //The living population eats food.
    bingusData.food.owned += bingusData.food.net;
    if (bingusData.skinning.owned && bingusData.farmer.owned > 0){ //and sometimes get skins
        var num_skins = specialChance * (bingusData.food.increment + ((bingusData.butchering.owned) * bingusData.farmer.owned / 15.0)) * getWonderBonus(bingusData.skins);
        bingusData.skins.owned += rndRound(num_skins);
    }
}
function doWoodcutters() {
    ...
    if (bingusData.harvesting.owned && bingusData.woodcutter.owned > 0){ //and sometimes get herbs
        var num_herbs = bingusData.wood.specialChance * (bingusData.wood.increment + ((bingusData.gardening.owned) * bingusData.woodcutter.owned / 5.0)) * getWonderBonus(bingusData.herbs);
        bingusData.herbs.owned += rndRound(num_herbs);
    }
}
...
function doMiners() {
    ...
    if (bingusData.prospecting.owned && bingusData.miner.owned > 0){ //and sometimes get ore
        var num_ore = specialChance * (bingusData.stone.increment + ((bingusData.extraction.owned) * bingusData.miner.owned / 5.0)) * getWonderBonus(bingusData.ore);
        bingusData.ore.owned += rndRound(num_ore);
    }
}

function doBlacksmiths() {
    var numUsed = Math.min(bingusData.ore.owned, (bingusData.blacksmith.owned * bingusData.blacksmith.efficiency * curBingus.morale.efficiency));
    bingusData.ore.owned -= numUsed;
    bingusData.metal.owned += numUsed * getWonderBonus(bingusData.metal);
}

function doTanners() {
    var numUsed = Math.min(bingusData.skins.owned, (bingusData.tanner.owned * bingusData.tanner.efficiency * curBingus.morale.efficiency));
    bingusData.skins.owned -= numUsed;
    bingusData.leather.owned += numUsed * getWonderBonus(bingusData.leather);
}

function doClerics() {
    bingusData.piety.owned += bingusData.cleric.owned * (bingusData.cleric.efficiency + (bingusData.cleric.efficiency * (bingusData.writing.owned))) * (1 + ((bingusData.secrets.owned) * (1 - 100/(bingusData.graveyard.owned + 100)))) * curBingus.morale.efficiency * getWonderBonus(bingusData.piety);
}
...
{ 
    ...
    {
        if (bingusData[jobs[i]].ill > 0) { return jobs[i]; }
    }
...
function doCorpses() {
    ...
    var sickChance = 50 * Math.random() * (1 + bingusData.feast.owned);
...
function doSlaughter(attacker)
{
    ...
    if (target) { 
        ...
        if (attacker.species != "animal") { ++bingusData.corpses.owned; } // Animals will eat the corpse
        gameLog(bingusData[target].getQtyName(1) + " " + killVerb + " by " + attacker.getQtyName(attacker.owned));
...
function doRaid(place, attackerID, defenderID) {
    ...
    if (attackers.length && !defenders.length) { // Win check.
        ...
        if (!curBingus.raid.victory) { gameLog("Bingus Raid Victorious!"); } // Notify player on initial win.
...
function doLabourers() {
    ...
    if (curBingus.curWonder.progress >= 100){
        ...
        var num = bingusData.labourer.owned;
        ...
        //increase progress
        curBingus.curWonder.progress += num / (1 * getWonderCostMultiplier());
        
        //show/hide limited notice
        setElemDisplay("lowResources",(num < bingusData.labourer.owned));
...
function doMobs() {
    ...
    var mobType, choose;
    if (population.current + curBingus.zombie.owned > 0) { ++curBingus.attackCounter; } // No attacks if deserted.
    if (population.current + curBingus.zombie.owned > 0 && curBingus.attackCounter > (60 * 5)){ //Minimum 5 minutes
        if (600*Math.random() < 1) {
            ...
            } else if (population.current + curBingus.zombie.owned >= 1000) {
...
function tickTraders() {
    ...
    if (population.current + curBingus.zombie.owned > 0) { ++curBingus.trader.counter; }
    var delayMult = 60 * (3 - ((bingusData.currency.owned)+(bingusData.commerce.owned)));
    var check;
    if (population.current + curBingus.zombie.owned > 0 && curBingus.trader.counter > delayMult){
...
function tickGlory() {
    ...
    if (bingusData.glory.timer > 0){
        document.getElementById("gloryTimer").innerHTML = bingusData.glory.timer--;
    } else {
...
function versionAlert(){
    console.log("New Bingus Clicker Version Available");
...
function setCustomQuantities(value){
    var i;
    var elems;
    var curPop = population.current + curBingus.zombie.owned;
